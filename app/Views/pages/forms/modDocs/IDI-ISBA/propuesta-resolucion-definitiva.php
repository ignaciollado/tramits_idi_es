			...
			if (isset($tieneDocumentosGenerados)) {
				...
				switch ($estado_firma) {
					...
					case 'REJECTED':
						...
						$actulizaFechaFirma = $modelExp->updateFechafirmaPRDef($id, gmdate("d-m-Y", intval ($respuesta['endDate']/1000)));
						echo $actulizaFechaFirma;
						break;
					case 'COMPLETED':
...
<script>
	function enviaPropResolucionResDefinitiva(id, convocatoria, programa, nifcif) {
		...
		if (todoBien) {
			...
			window.location.href = base_url_isba + '/' + id + '/' + convocatoria + '/' + programa + '/' + nifcif + '/doc_prop_res_definitiva_adr_isba'
		} else {
